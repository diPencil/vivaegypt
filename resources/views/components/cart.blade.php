...
@push('js_section')
    <script>
        function cartList() {
            $('#minicartContent').html("{{ __('translate.Please wait...') }}");
            $.ajax({
                url: `{{ route('cart.cart_items') }}`,
                method: 'GET',
                success: function(data) {
                    $('#minicartContent').html(data.view);
                },
                error: function() {
                    $('#minicartContent').html('<div class="text-center">' + @json(__('translate.Error loading cart items')) + '</div>');
                }
            });
        }
        ...
        function removeFromCart(cartId) {
            $.ajax({
                ...
                error: function() {
                    $('#minicartContent').html(@json(__('translate.Error loading cart items')));
                }
            });
        }
    </script>
