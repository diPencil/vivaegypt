                                            ...
                                            <div class="translation_box">
                                                <ul>
                                                    @foreach ($language_list as $language)
                                                        <li><a
                                                                href="{{ route('admin.menus.edit', ['id' => $menu->id, 'lang_code' => $language->lang_code]) }}">
                                                ...
                                                <form action="{{ route('admin.menus.add-item', $menu->id) }}"
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <select name="type" class="form-select" id="menuItemType">
                                                            ...
                                                            <option value="language">{{ __('translate.Language Switcher') }}
                                                            </option>
                                                        </select>
                                                    </div>
    ...
    <script>
        $(function() {
            ...
            $(document).on('click', '.edit-menu-item', function(e) {
                ...
                $.ajax({
                    url: '{{ url('admin/menu-items') }}/' + id + '/edit?lang_code={{ $lang_code }}',
                    type: 'GET',
                    dataType: 'json',
                    ...
                    error: function() {
                        toastr.error('{{ __('translate.Error fetching menu item data') }}');
                    }
                });
            });
            ...
            $('#saveMenuStructure').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            toastr.success('{{ __('translate.Menu structure updated successfully') }}');
                            setTimeout(function(){ location.reload(); }, 1000);
                        } else {
                            toastr.error('{{ __('translate.Error updating menu structure') }}');
                        }
                    },
                    error: function() {
                        toastr.error('{{ __('translate.Error updating menu structure') }}');
