...
if (!function_exists('render_menu_item')) {
    ...
    function render_menu_item($item, $args, $depth = 0) {
        ...
        // Handle Language Switcher type
        if ($item->type == 'language') {
            $languages = \Modules\Language\App\Models\Language::where('status', 1)->get();
            if ($languages->count() > 1) {
                $item_classes = ['menu-item-has-children', 'language-switcher-menu'];
                if ($item->css_class) {
                    $item_classes = array_merge($item_classes, explode(' ', $item->css_class));
                }
                
                $item_class_string = ' class="' . esc_attr(implode(' ', $item_classes)) . '"';
                $item_output = '<li' . $item_class_string . '>';
                $item_output .= '<a href="javascript:void(0)"><i class="fa-solid fa-globe mr-5"></i> ' . $item->title . '</a>';
                $item_output .= '<ul class="sub-menu">';
                foreach ($languages as $lang) {
                    $isActive = session()->get('front_lang') == $lang->lang_code ? 'active' : '';
                    $item_output .= '<li class="' . $isActive . '"><a href="' . route('language-switcher') . '?lang_code=' . $lang->lang_code . '">' . $lang->lang_name . '</a></li>';
                }
                $item_output .= '</ul></li>';
                return $item_output;
            }
