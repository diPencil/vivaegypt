...
class ThemeTestCommand extends Command
{
    ...
    protected function listThemes()
    {
        ...
        foreach ($possiblePaths as $path) {
            if (file_exists($path)) {
                $this->info("âœ“ Path exists: {$path}");
                if (is_dir($path)) {
                    $this->info("âœ“ Path is a directory");
                    $dirContent = scandir($path);
                    $this->info("Directory contents: " . implode(', ', array_filter($dirContent, function($item) {
                        return !in_array($item, ['.', '..']);
                    })));
                } else {
                    $this->error("âœ• Path is not a directory: {$path}");
                }
            } else {
                $this->error("âœ• Path does not exist: {$path}");
            }
        }
    ...
    protected function checkActiveTheme()
    {
        ...
        if (file_exists($themePath)) {
            $this->info("âœ“ Theme directory exists: {$themePath}");
        } else {
            $this->error("âœ• Theme directory does not exist: {$themePath}");
        }
    }
    ...
    protected function fixThemeStorage()
    {
        ...
        try {
            ...
            $this->info("âœ“ Updated database setting");
        } catch (\Exception $e) {
            $this->error("Database error: " . $e->getMessage());
            ...
            try {
                if (!Schema::hasTable('settings')) {
                    ...
                    $this->info("âœ“ Created settings table and inserted active_theme");
                }
        ...
        $this->info("âœ“ Updated settings file");
        
        // Test the theme system
        Theme::set($activeTheme);
        $this->info("âœ“ Theme set to: " . Theme::current());
