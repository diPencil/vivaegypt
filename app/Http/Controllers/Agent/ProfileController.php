...
class ProfileController extends Controller
{
    ...
    public function edit_profile()
    {
        ...
        return view('agent.edit_profile', ['user' => $user]);
    }
    ...
    public function update_agent_profile(BecomeAgentRequest $request)
    {
        $user = Auth::guard('web')->user();
        ...
        if ($request->hasFile('agent_logo')) {
            ...
            $old_agent_logo = $user->agent_logo;

            if ($old_agent_logo) {
                if (File::exists(public_path() . '/' . $old_agent_logo)) unlink(public_path() . '/' . $old_agent_logo);
            }

            $file = $request->file('agent_logo');
            $imageName = 'uploads/custom-images/' . Str::slug($user->agent_name) . '-' . now()->format('YmdHis') . '-' . rand(1000, 9999) . '.' . $file->getClientOriginalExtension();

            Image::make($file)->save(public_path($imageName));

            $user->agent_logo = $imageName;
            $user->save();
        }
    ...
    {
        return view('agent.account_delete');
    }
