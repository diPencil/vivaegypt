...
class LoginController extends Controller
{
    ...
    */
    ...
    public function store_login(Request $request){
        ...
        $custom_error = [
            'email.required' => trans('translate.Email or Username is required'),
            'password.required' => trans('translate.Password is required'),
        ];

        $this->validate($request, $rules, $custom_error);

        $staff = User::where('email', $request->email)->orWhere('username', $request->email)->first();
        ...
        if ($staff && $staff->isStaff()) {
            ...
            if (Auth::guard('web')->attempt(['email' => $staff->email, 'password' => $request->password], $request->remember)) {
                $notify_message = trans('translate.Login successfully');
                $notify_message = array('message' => $notify_message, 'alert-type' => 'success');

                return redirect()->route('staff.dashboard')->with($notify_message);
            }
                ...
                if($user->email_verified_at != null){
                    ...
                    if(Hash::check($request->password, $user->password)){
                        if(Auth::guard('web')->attempt(['email' => $user->email, 'password' => $request->password], $request->remember)){
                            ...
                            $notify_message = array('message' => $notify_message, 'alert-type' => 'success');
                            if($user->is_seller == 1){
                                return redirect()->route('agent.dashboard')->with($notify_message);
                            }else{
        ...
        $notify_message = trans('translate.Email or Username not found');
        $notify_message = array('message' => $notify_message, 'alert-type' => 'error');
