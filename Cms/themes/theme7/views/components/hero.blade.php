@php
    $theme7_hero = getContent('theme7_hero.content', true);
    $theme7_destinations = destinations();
    $theme7_service_type = serviceTypeTab();
@endphp

<!-- tg-hero-area-start -->
<div class="tg-hero-area tg-hero-5-spacing z-index-1 p-relative include-bg"
    data-background="{{ asset(getSingleImage($theme7_hero, 'background_image')) }}">
    <div class="tg-hero-top-shadow"></div>
    <div class="tg-hero-5-top-shadow"></div>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="tg-hero-3-content tg-hero-5-content text-center mb-45">
                    <h4 class="tg-hero-3-subtitle mb-15">
                        {{ getTranslatedValue($theme7_hero, 'sub_title') }}
                        <span class="tg-hero-5-title-shape">
                            <svg width="262" height="15" viewBox="0 0 262 15" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M1 9.72727L89.3301 5L70.8656 13L123.764 5L118.774 13L178.159 5V13L261 1"
                                    stroke="white" stroke-width="2" stroke-linecap="round" />
                            </svg>
                        </span>
                    </h4>
                    <h2 class="tg-hero-3-title">
                        {!! strip_tags(clean(getTranslatedValue($theme7_hero, 'title')), '<br>') !!}
                    </h2>
                </div>
            </div>
            <div class="col-12" x-data="bookingForm()">
                <div class="tg-booking-form-item tg-booking-5-form-item">
                        ...
                        <div class="tg-booking-form-input-group d-flex align-items-end justify-content-between">
                            <div class="tg-booking-form-parent-inner tg-hero-quantity p-relative mr-15 mb-15">
                                ...
                                <div class="tg-booking-form-location-list tg-booking-quantity-active">
                                    <ul class="scrool-bar scrool-height pr-5">
                                        @foreach ($theme7_destinations as $key => $destination)
                                            <li
                            ...
                            <div class="tg-booking-form-parent-inner mr-15 mb-15">
                                ...
                                <div class="tg-booking-add-input-date p-relative">
                                    <input x-model="check_in" class="input datepicker" name="datetime-local"
                            ...
                            <div class="tg-booking-form-parent-inner mr-15 mb-15">
                                ...
                                <div class="tg-booking-add-input-date p-relative">
                                    <input x-model="check_out" class="input datepicker" name="datetime-local"
                                        type="text" placeholder="{{ __("translate.Select Date") }}">
                                    <span>
                                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div class="tg-booking-form-search-btn mb-15">
                                <button class="bk-search-button" type="submit">{{ __('translate.Search') }}
                                    <span class="ml-5">
                                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                ...
                @if ($theme7_service_type->count() > 0)
                <div class="d-flex justify-content-center flex-wrap mt-4" style="gap: 15px;">
                    @foreach ($theme7_service_type as $key => $service_type)
                        <button type="button"
                            @click="service_slug = '{{ $service_type->slug }}'"
                            style="display: inline-flex; align-items: center; justify-content: center; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.25); border-radius: 40px; padding: 10px 24px; font-size: 15px; font-weight: 500; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.05);"
                            :style="service_slug === '{{ $service_type->slug }}' ? 'background: var(--tg-theme-primary) !important; color: #fff !important; border-color: var(--tg-theme-primary) !important; box-shadow: 0 6px 20px rgba(0,0,0,0.15) !important;' : 'background: rgba(255, 255, 255, 0.15) !important; color: #fff !important; border-color: rgba(255, 255, 255, 0.25) !important;'">
                            @if ($service_type->image)
                                <img src="{{ asset($service_type->image) }}" width="20" height="20" class="me-2" style="filter: brightness(0) invert(1);" alt="">
                            @elseif ($service_type->icon)
                                <i class="{{ $service_type->icon }} me-2" style="filter: brightness(0) invert(1);"></i>
                            @endif
    ...
    <script>
        function bookingForm() {
            return {
                ...
                adults: '',
                children: '',
                service_slug: '{{ $theme7_service_type->count() > 0 ? $theme7_service_type->first()->slug : '' }}',
