    ...
    <div class="container">
            ...
            <div class="col-lg-12">
                <div class="tg-listing-menu-nav project__menu-nav mb-40 wow fadeInUp" data-wow-delay=".5s"
                    ...
                    <button class="active" data-filter="*">
                        ...
                        <span>{{ __('brand.home_tours_all') }}</span>
                    </button>
                    ...
                    @foreach ($theme1_service as $key => $service_type)
                        <button data-filter=".type-{{ $service_type->id }}">
                            ...
                            <span>{{ $service_type->localized_name }}</span>
                        </button>
                    @endforeach
        ...
        <div class="row project-active-two">
            @foreach ($theme1_popular_services as $key => $service)
                <div
                    class="col-xxl-3 col-xl-4 col-lg-4 col-md-6 grid-item grid-sizer type-{{ $service->service_type_id }}">
                    <div class="tg-listing-card-item mb-30">
                        <div class="tg-listing-card-thumb fix mb-15 p-relative">
                            <a href="{{ route('front.tourbooking.services.show', ['slug' => $service?->slug]) }}">
                                ...
                                @endphp
                                @if ($service?->is_new == 1)
                                        <span class="tg-listing-item-price-discount shape {{ $badgeCount > 0 ? ($badgeCount == 1 ? 'offer-btm' : 'offer-btm-2') : '' }}"
                                            style="background-image: url('{{ asset('frontend/assets/img/shape/price-shape-2.png') }}')"><span>{{ __('brand.label_new') }}</span></span>
                                        @php $badgeCount++; @endphp
                                @endif

                                @if ($service?->discount_price)
                                    <span class="tg-listing-item-price-discount shape-2 {{ $badgeCount > 0 ? ($badgeCount == 1 ? 'offer-btm' : 'offer-btm-2') : '' }}"
                                        style="background-image: url('{{ asset('frontend/assets/img/shape/offter.png') }}')"> <span>{{ __('brand.label_limited_offer') }}</span> </span>
                                        @php $badgeCount++; @endphp
                                @endif

                                @if ($service?->is_featured == 1)
                                    <span class="tg-listing-item-price-discount shape-3 {{ $badgeCount > 0 ? ($badgeCount == 1 ? 'offer-btm' : 'offer-btm-2') : '' }}"
                                        ...
                                        <span>{{ __('brand.label_featured') }}</span>
                                    </span>
                                    @php $badgeCount++; @endphp
                                @endif
                            ...
                            <div class="tg-listing-card-price d-flex align-items-end justify-content-between">
                                ...
                                <div class="tg-listing-card-review space">
                                    ...
                                    <span class="tg-listing-rating-percent">
                                        {{ $service?->active_reviews_count ?? 0 }}
                                        {{ $service?->active_reviews_count > 1 ? __('translate.Reviews') : __('translate.Review') }}
                                    </span>
                                </div>
                            </div>
                        </div>
...
@push('style_section')
    <style>
        ...
        /* RTL Fixes */
        [dir="rtl"] .tg-listing-card-review {
            display: flex;
            align-items: center;
        ...
        [dir="rtl"] .tg-listing-rating-percent {
            direction: rtl;
            unicode-bidi: embed;
        }
        [dir="rtl"] .tg-listing-card-price-wrap {
            margin-right: 0 !important;
            margin-left: 0 !important;
            transform: scaleX(-1);
            border-radius: 12px 0 0 0 !important;
            padding-right: 50px !important; /* Fine-tuned position */
        }
        [dir="rtl"] .tg-listing-card-currency-amount {
            transform: scaleX(-1);
            display: inline-block;
        }
        [dir="rtl"] .tg-listing-item-wishlist {
            right: auto !important;
            left: 15px !important;
        }
        [dir="rtl"] .tg-listing-item-price-discount {
            left: auto !important;
            right: 0 !important;
            transform: scaleX(-1);
            border-radius: 5px 0 0 5px !important;
            padding: 6px 14px 6px 20px !important; /* Swapped padding for RTL */
        }
        [dir="rtl"] .tg-listing-item-price-discount span,
        [dir="rtl"] .tg-listing-item-price-discount svg {
            transform: scaleX(-1) !important;
            display: inline-block !important;
        }
        [dir="rtl"] .tg-listing-item-price-discount svg {
            margin-right: 8px !important; /* Space between thunderbolt and text */
            margin-left: 0 !important;
        }
        [dir="rtl"] .tg-listing-item-price-discount.offer-btm {
            top: 47px !important;
        }
        [dir="rtl"] .tg-listing-item-price-discount.offer-btm-2 {
            top: 77px !important;
        }
        [dir="rtl"] .tg-listing-card-duration-tour span {
        ...
        [dir="rtl"] .tg-listing-card-duration-map svg,
        [dir="rtl"] .tg-listing-card-duration-time svg {
            margin: 0 !important;
            order: -1 !important;
        }
        [dir="rtl"] .tg-listing-card-review {
        ...
        [dir="rtl"] .tg-listing-rating-icon {
            order: 1 !important;
            margin: 0 !important;
        }
        [dir="rtl"] .tg-listing-rating-percent {
