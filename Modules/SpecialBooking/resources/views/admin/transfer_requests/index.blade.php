...
@section('title')
    <title>{{ __('translate.Transfer Request List') }}</title>
@endsection

@section('body-header')
    <h3 class="crancy-header__title m-0">{{ __('translate.Transfer Request List') }}</h3>
    <p class="crancy-header__text">{{ __('translate.Special Bookings') }} >> {{ __('translate.Transfer Request List') }}</p>
@endsection
                            ...
                            <div class="crancy-table crancy-table--v3 mg-top-30">
                                <div class="crancy-customer-filter">
                                    <div class="crancy-header__form crancy-header__form--customer create_new_btn_inline_box">
                                        <h4 class="crancy-product-card__title">{{ __('translate.Transfer Request List') }}</h4>
                                    </div>
                                </div>
                                            ...
                                            @foreach ($requests as $request)
                                                <tr>
                                                    ...
                                                    <td class="crancy-table__column-2 crancy-table__data-2">
                                                        ...
                                                        <br><small>{{ ucfirst($request->transfer_type) }}</small>
                                                    </td>
                                                    <td class="crancy-table__column-2 crancy-table__data-2">{{ $request->pickup_date }} <small>{{ $request->pickup_time }}</small></td>
                                                    <td class="crancy-table__column-2 crancy-table__data-2">
                                                        @php($badge = [
                                                    ...
                                                    <td class="crancy-table__column-2 crancy-table__data-2">
                                                        <a href="{{ dashboard_route('admin.special-booking.transfer-requests.show', $request->id) }}"
