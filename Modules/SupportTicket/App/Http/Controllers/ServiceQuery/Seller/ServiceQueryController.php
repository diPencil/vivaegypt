...
class ServiceQueryController extends Controller
{
    ...
    public function index()
    {
        ...
        })->where('admin_type', 'agent')->latest()->get();
    ...
    public function show($ticket_id)
    {
        ...
        $support_ticket = SupportTicket::withWhereHas('service', function ($query) use ($user) {
            $query->where('user_id', $user->id);
    ...
    public function close($id)
    {
        ...
        $notify_message = array('message' => $notify_message, 'alert-type' => 'success');
        return redirect()->route('agent.supports')->with($notify_message);
    }
}
