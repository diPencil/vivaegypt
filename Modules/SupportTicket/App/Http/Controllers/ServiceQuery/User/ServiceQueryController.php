...
class ServiceQueryController extends Controller
{
    ...
    public function create()
    {
        ...
        $confirm_booking = Booking::query()
            ->select('id', 'service_id', 'booking_status')
    ...
    public function store(Request $request)
    {
        ...
        $support_ticket->admin_type = 'agent';
        ...
        if ($request->hasFile('documents')) {
            foreach ($request->documents as $index => $request_file) {
                ...
                $file_name = 'agent-support-' . time() . $index . '.' . $extention;
        ...
        return redirect()->route('user.agent-support.show', $support_ticket->ticket_id)->with($notify_message);
    }
