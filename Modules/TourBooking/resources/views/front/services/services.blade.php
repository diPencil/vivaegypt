...
@section('title')
    <title>{{ __('translate.Services') }} - Viva Egypt Travel</title>
    ...
    <main>
        ...
        <div x-data="data">
            ...
            <div class="tg-booking-form-area p-relative z-index-1 tg-listing-booking-shadow">
                <img class="tg-booking-4-shape d-none d-lg-block" src="{{ asset('frontend/assets/img/shape/hill-4.png') }}"
                    alt="shape">
                <img class="tg-booking-4-shape-2 d-none d-lg-block" src="{{ asset('frontend/assets/img/shape/hill-5.png') }}"
                    alt="shape">
                <div class="container">
                    <div class="row">
                        <div class="col-12">
                                    ...
                                    <div class="tg-booking-form-input-group d-flex align-items-end justify-content-between">
                                            ...
                                            <div class="tg-booking-form-location-list tg-booking-quantity-active">
                                                <ul class="scrool-bar scrool-height pr-5">
                                                    @foreach ($destinations as $key => $destination)
                                                        <li
                                                            @click="selectDestination(`{{ $destination->id }}`, `{{ $destination->localized_name }}`)">
                                        ...
                                        <div class="tg-booking-form-parent-inner mr-15 mb-15">
                                            ...
                                            <div class="tg-booking-add-input-date p-relative">
                                                <input x-model="bookingForm.checkIn" class="input datepicker"
                                        ...
                                        <div class="tg-booking-form-parent-inner mr-15 mb-15">
                                            ...
                                            <div class="tg-booking-add-input-date p-relative">
                                                <input x-model="bookingForm.checkOut" class="input datepicker"
                                                    name="check_out" type="text" placeholder="{{ __("translate.Check out") }}">
                                                <span>
                                                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                                                        xmlns="http://www.w3.org/2000/svg">
                                                        <path
                        ...
                        <div class="col-xl-3 col-lg-4 order-last order-lg-first">
                            <div class="tg-filter-sidebar mb-40 top-sticky">
                                <div class="tg-filter-item">
                                    ...
                                    <div class="tg-filter-section-service-type" x-data="{ showPropertyType: false }">
                                        <h4 class="tg-filter-title mb-15">{{ __('translate.Service Type') }}</h4>
                                        ...
                                        @if (count($serviceTypes) > 4)
                                            <div class="tg-filter-seemore mt-2 cp select-none"
                                                ...
                                                <span class="more"
                                                    x-text="showPropertyType ? '{{ __('translate.See Less') }}' : '{{ __('translate.See More') }}'"></span>
                                            </div>
                                        @endif
                                    ...
                                    <div class="tg-filter-section-currency" x-data="{ showCurrency: false }">
                                        <h4 class="tg-filter-title mb-15">{{ __('translate.Currency') }}</h4>
                                        @if (isset($currency_list) && $currency_list->count() > 0)
                                            <div class="tg-filter-list tg-filter-currency-radios">
                                                <ul>
                                                    @foreach ($currency_list as $key => $currency)
                                                        ...
                                                        @endphp
                                                        <li x-show="showCurrency || {{ $key }} < 4" x-transition>
                                                            <div class="checkbox d-flex">
                                                                <input type="radio" name="sidebar_filter_currency"
                                                                    class="tg-checkbox" value="{{ $currency->currency_code }}"
                                                                    id="currency_radio_{{ $key }}"
                                                                    {{ Session::get('currency_code') == $currency->currency_code ? 'checked' : '' }}
                                                                    @change="if ($event.target.checked && $event.target.value !== @js(Session::get('currency_code'))) { window.location.href = @js(route('currency-switcher')) + '?currency_code=' + encodeURIComponent($event.target.value); }">
                                                                <label for="currency_radio_{{ $key }}"
                                                                    class="tg-label">{{ $currencyRowLabel }}</label>
                                                            </div>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            ...
                                            @if ($currency_list->count() > 4)
                                                <div class="tg-filter-seemore mt-2 cp select-none"
                                                    @click="showCurrency = !showCurrency">
                                                    <span class="plus">
                                                        <i
                                                            :class="showCurrency ? 'fa-solid fa-minus' :
                                                                'fa-sharp fa-solid fa-plus'"></i>
                                                    </span>
                                                    <span class="more"
                                                        x-text="showCurrency ? '{{ __('translate.See Less') }}' : '{{ __('translate.See More') }}'"></span>
                                                </div>
                                            @endif
                                        @endif
                                    ...
                                    <div class="tg-filter-price-input">
                                        <h4 class="tg-filter-title mb-20">{{ __('translate.Price By Filter') }}</h4>
                                        <div class="d-flex align-items-center">
                                            <input class="input no-arrow" x-model="filters.min_price" type="number"
                                                placeholder="{{ __('translate.Min Price') }}">
                                            <span class="dvdr">
                                            ...
                                            <input class="input no-arrow" x-model="filters.max_price" type="number"
                                                placeholder="{{ __('translate.Max Price') }}">
                                        </div>
                                    </div>
                                    <span class="tg-filter-border mt-25 mb-25"></span>

                                    <div x-data="{ showAmenity: false }">
                                        <h4 class="tg-filter-title mb-15">{{ __('translate.Amenities') }}</h4>
                                        <div class="tg-filter-list">
                                            <ul>
                                                    ...
                                                    <li x-show="showAmenity || {{ $key }} < 4" x-transition>
                                                        <div class="checkbox d-flex">
                                                            ...
                                                            <label for="amenity_{{ $key }}" class="tg-label">
                                                                {{ $amenity?->translation?->name ?? $amenity?->translations?->first()?->name ?? $amenity?->slug }}
                                                            </label>
                                                        </div>
                                                    </li>
                                                @endforeach
                                        ...
                                        @if (count($amenities) > 4)
                                            <div class="tg-filter-seemore mt-2 cp select-none"
                                                ...
                                                <span class="more"
                                                    x-text="showAmenity ? '{{ __('translate.See Less') }}' : '{{ __('translate.See More') }}'"></span>
                                            </div>
                                        @endif
                                    ...
                                    <h4 class="tg-filter-title mb-15">{{ __('translate.Top Reviews') }}</h4>
                                    <div class="tg-filter-list">
                                        <ul>
                                            @for ($i = 5; $i >= 1; $i--)
                                                <li>
                                    ...
                                    <div x-data="{ showMoreLanguages: false }">
                                        <h4 class="tg-filter-title mb-15">{{ __('translate.Language') }}</h4>
                                        <div class="tg-filter-list">
                                            <ul>
                                        ...
                                        @if (count($languages) > 4)
                                            <div class="tg-filter-seemore mt-2 cp select-none"
                                                ...
                                                <span class="more"
                                                    x-text="showMoreLanguages ? '{{ __('translate.See Less') }}' : '{{ __('translate.See More') }}'"></span>
                                            </div>
                                        @endif
                                    </div>
                                            ...
                                            <div
                                                ...
                                                <div class="tg-listing-select-price ml-10">
                                                    <select id="sortSelect" class="select" name="sort_by">
                                                        <option value="default">{{ __('translate.Default') }}</option>
                                                        <option value="latest">{{ __('translate.Latest') }}</option>
                                                        <option value="oldest">{{ __('translate.Oldest') }}</option>
                                                        <option value="price_low">{{ __('translate.Price Low') }}</option>
                                                        <option value="price_high">{{ __('translate.Price High') }}</option>
                                                        <option value="trending">{{ __('translate.Trending') }}</option>
                                                        <option value="popular">{{ __('translate.Popular') }}</option>
                                                        <option value="location_asc">{{ __('translate.Location A-Z') }}</option>
                                                        <option value="location_desc">{{ __('translate.Location Z-A') }}</option>
                                                    </select>
                                                </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('data', () => ({
                ...
                defaultBookingForm: {
                    destination_id: '',
                    destination: @json(__('translate.Where are you going . . .')),
                    checkIn: '',
                    checkOut: '',
                    rooms: 0,
                    adults: 0,
                ...
                filters: {
                    ...
                    only_offers: `{{ isset($only_offers) && $only_offers ? 'true' : 'false' }}`,
                },
                defaultFilters: {
                    search: '',
                    service_type_ids: [],
...
@push('style_section')
    <style>
        /* Hero Styles */
        .destination-hero-section { overflow: hidden; position: relative; }
