...
{
    /**
     * Preferred service type order for frontend filters and dropdowns.
     *
    ...
    /**
     * Sort a collection by the preferred frontend order.
     */
    private function sortByPreferredOrder(Collection $serviceTypes): Collection
    {
        return $serviceTypes
            ->sortBy(function (ServiceType $serviceType) {
                $name = trim((string) $serviceType->name);

                return $this->preferredOrder[$name] ?? 999;
            })
            ->values();
    }
    ...
    {
        return $this->sortByPreferredOrder(ServiceType::with('translation')
            ->active()
    ...
    public function getActiveNameId(): Collection
    {
        return $this->sortByPreferredOrder(ServiceType::with('translation')
            ...
            ->orderBy('display_order')
            ->get())
            ->map(function (ServiceType $serviceType) {
                $serviceType->name = $serviceType->localized_name;
    ...
    public function saveTranslation(ServiceType $serviceType, string $locale, array $data): ServiceTypeTranslation
    {
        $translation = ServiceTypeTranslation::updateOrCreate(
            ['service_type_id' => $serviceType->id, 'lang_code' => $locale],
            $data
        );
