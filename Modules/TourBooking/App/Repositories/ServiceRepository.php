...
final class ServiceRepository
{
    ...
    public function getAllFilters(array $filters = [])
    {
        ...
        if (($filters['booking_visibility'] ?? null) === 'manageable') {
        ...
        } elseif (($filters['booking_visibility'] ?? null) === 'inactive_drafts') {
            $query->where('status', false)
                ->whereDoesntHave('bookings');
        } elseif (($filters['booking_visibility'] ?? null) === 'archived') {
            $query->where('status', false)
                ->whereHas('bookings');
        } elseif (isset($filters['status'])) {
            $query->where('status', $filters['status']);
        } elseif (($filters['active_only'] ?? true) === true) {
