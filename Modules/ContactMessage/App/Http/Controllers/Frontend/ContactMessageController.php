...
class ContactMessageController extends Controller
{
    ...
    public function store_contact_message(ContactMessageRequest $request)
    {
        ...
        try{
            ...
            if ($request->filled('seller_id') && (int) $request->seller_id > 0) {
                $seller = User::findOrFail((int) $request->seller_id);

                Mail::to($seller->email)->send(new SendContactMessage($message, $subject, $request->email, $request->name));
            }else{
        ...
        $notify_message=array('message'=>$notify_message,'alert_type'=>'success');
